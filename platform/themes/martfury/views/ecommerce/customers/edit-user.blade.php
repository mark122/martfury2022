...
@section('content')
    <div class="ps-my-account">
        {{--    <div class="container">--}}
        <form class="ps-form--account ps-tab-root" method="POST" action="{{ route('customer.update.user',$customer->id) }}">
            @csrf
            <div class="ps-form__content">
                ...
                <div class="row center">
                    ...
                    <div class="col-md-6">
                        <input type="radio" id="html" name="ranking" value="3" @if($customer->ranking == 3)checked @endif>
                        <label for="html"><b>Site Approval</b></label><br>
                    </div>
                    <div class="col-md-6">
                        <input type="radio" id="css" name="ranking" value="4" @if($customer->ranking == 4)checked @endif>
                        <label for="css"><b>Site Representative</b></label><br>
                    </div>
                </div>
                <div class="form-group">
                    <input class="form-control" name="name" id="txt-name" type="text" value="{{ $customer->name }}" placeholder="{{ __('Your Name') }}">
                    @if ($errors->has('name'))
                        <span class="text-danger">{{ $errors->first('name') }}</span>
                    @endif
                </div>
                <div class="form-group">
                    <input class="form-control" name="email" id="txt-email" type="email" value="{{ $customer->email }}" autocomplete="email" placeholder="{{ __('Your Email') }}" readonly>
                    @if ($errors->has('email'))
...
{{--                </div>--}}
                ...
                <div class="form-group submit">
                    <button class="ps-btn ps-btn--fullwidth" type="submit">{{ __('Save') }}</button>
                </div>
