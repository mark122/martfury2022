                ...
                <div class="col-md-9">
                    ...
                    <table class="table payment-method-item">
                        ...
                        <tr class="border-pay-row">
                            <td class="border-pay-col"><i class="fa fa-theme-payments"></i></td>
                            <td style="width: 20%;">
                                <img class="filter-black" src="{{ url('vendor/core/plugins/payment/images/hitpay.png') }}" alt="Hitpay">
                            </td>
                            <td class="border-right">
                                <ul>
                                    <li>
                    ...
                    <div class="table-responsive">
                     <table class="table payment-method-item">
                         ...
                         @php $sfcaStatus =  setting('payment_send_for_corporate_approval_status'); @endphp

                         <tr class="bg-white">
                             <td colspan="3">
                                 <div class="float-start" style="margin-top: 5px;">
                                     <div class="payment-name-label-group">
                                         @if ($sfcaStatus != 0)<span class="payment-note v-a-t">{{ trans('plugins/payment::payment.use') }}:</span>@endif <label class="ws-nm inline-display method-name-label">Send for Corporate approval</label>
                                     </div>
                                 </div>
                                 <div class="float-end">
                                     <a class="btn btn-secondary toggle-payment-item edit-payment-item-btn-trigger @if ($sfcaStatus == 0) hidden @endif">{{ trans('plugins/payment::payment.edit') }}</a>
                                     <a class="btn btn-secondary toggle-payment-item save-payment-item-btn-trigger @if ($sfcaStatus == 1) hidden @endif">{{ trans('plugins/payment::payment.settings') }}</a>
                                 </div>
                             </td>
                         </tr>
                         <tr class="paypal-online-payment payment-content-item hidden">
                             <td class="border-left" colspan="3">
                                 {!! Form::open() !!}
                                 {!! Form::hidden('type', 'send_for_corporate_approval', ['class' => 'payment_type']) !!}
                                 <div class="col-sm-12 mt-2">
                                     <div class="well bg-white">
                                         <div class="form-group mb-3">
{{--                                             @dd(\Botble\Payment\Enums\PaymentMethodEnum::COD())--}}
                                             <label class="text-title-field" for="payment_send_for_corporate_approval_name">{{ trans('plugins/payment::payment.method_name') }}</label>
                                             <input type="text" class="next-input" name="payment_send_for_corporate_approval_name" id="payment_send_for_corporate_approval_name" data-counter="400" value="{{ setting('payment_send_for_corporate_approval_name', 'Send For Corporate Approval') }}">
                                         </div>
                                         <div class="form-group mb-3">
                                             <label class="text-title-field" for="payment_send_for_corporate_approval_description">{{ trans('plugins/payment::payment.payment_method_description') }}</label>
                                             {!! Form::editor('payment_send_for_corporate_approval_description', setting('payment_send_for_corporate_approval_description')) !!}
                                         </div>
                                         {!! apply_filters(PAYMENT_METHOD_SETTINGS_CONTENT, null, 'payment_send_for_corporate_approval_name', \Botble\Payment\Enums\PaymentMethodEnum::SendForCoporateApproval()->label()) !!}
                                     </div>
                                 </div>
                                 <div class="col-12 bg-white text-end">
                                     <button class="btn btn-warning disable-payment-item @if ($sfcaStatus == 0) hidden @endif" type="button">{{ trans('plugins/payment::payment.deactivate')  }}</button>
                                     <button class="btn btn-info save-payment-item btn-text-trigger-save @if ($sfcaStatus == 1) hidden @endif" type="button">{{ trans('plugins/payment::payment.activate') }}</button>
                                     <button class="btn btn-info save-payment-item btn-text-trigger-update @if ($sfcaStatus == 0) hidden @endif" type="button">{{ trans('plugins/payment::payment.update') }}</button>
                                 </div>
                                 {!! Form::close() !!}
                             </td>
                         </tr>
