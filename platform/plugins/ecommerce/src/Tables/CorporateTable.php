...
class CorporateTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    public function query()
    {
        $query = $this->repository->getModel()->select([
            ...
            'status',
            'confirmed_at',
        ])->when(\request('code'), function ($qp) {
                return $qp->where('corporate_code', request('code'));
        })->when(\request('corporate-user'), function ($qp) {
                return $qp->where('corporate_code',request('corporate-user'))->where('ranking','2');
        })->when(\request('a-r'), function ($qp) {
                 return $qp->where('corporate_code',request('a-r'))->whereIn('ranking',['3','4']);
        });
