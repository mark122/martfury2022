...
class CorporateOrderTable extends TableAbstract
{
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('actual_amount', function ($item) {
                return format_price($item->amount+$item->discount_amount);
            })
    ...
    public function query()
    {
        $query = $this->repository->getModel()
            ...
            ->with(['user', 'payment'])
            ->where('is_finished', 1)->whereHas('user', function ($q) {
