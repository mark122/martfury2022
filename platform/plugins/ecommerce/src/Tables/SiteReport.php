...
class SiteReport extends TableAbstract
{
    ...
    public function ajax()
    {
        ...
        return date_format($item->updated_at,'Y-m-d H:i:s');
    });
    ...
    public function query()
    {
        $query = $this->repository->getModel()
            ...
            ->with(['user', 'payment'])->whereHas('user', function ($q) {
                return $q->where('ranking','>=', 2);
            })
            ->where('is_finished', 1);

        if (request('code')){
           $cust = Customer::where('corporate_code',request('code'))->pluck('id')->toArray();
            $query->whereIn('user_id',$cust);
        }
    ...
    public function columns()
    {
        $columns = [
            ...
            'updated_at'      => [
                'title' => trans('Date Time Approve/Reject'),
                'width' => '100px',
                'class' => 'text-start',
            ],
//            'user_id' => [
