...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function register(Request $request, BaseHttpResponse $response)
    {
        ...
        if (isset($request->corporate_code)){
        $corporate_code = CorporateCode::where('list_of_corporate_code', $request->corporate_code)->first();
            if (!empty($corporate_code)) {
                $underCodeCorporate = Customer::where('corporate_code', $request->corporate_code)->where('ranking', 2)->count();
                if ($underCodeCorporate < $corporate_code->member_under ) {
                    $customer->update(['corporate_code' => $request->corporate_code, 'ranking' => 2]);
                }
            }
        }
