...
@section('content')
   ...
   @if(auth('customer')->user()->ranking == 1)
       @php
           $dis = (Cart::instance('cart')->rawSubTotal()*(auth('customer')->user()->subcontractor_dis)) / 100 ;
       @endphp
       <div class="row">
           <div class="col-6">
           <p>Discount ({{auth('customer')->user()->subcontractor_dis}}%):</p>
       </div>
           <div class="col-6 float-end">
               <p class="price-text tax-price-text">{{ format_price($dis) }}</p>
           </div>
       </div>
   @endif
   @endif
...
<form action="{{ route('payments.checkout') }}" method="post">
   ...
   <div>
       ...
       <input type="hidden" name="delivery_dt" id="delivery_dt">
   ...
   <br>
{{--     @dd(is_plugin_active('marketplace'))--}}
   @if (!is_plugin_active('marketplace'))
       ...
       <br>
   @endif
{{--@dd($defaultShippingOption)--}}
    <div
        class="row @if (old('shipping_method', 'default') == $defaultShippingMethod && old('shipping_option', '1') == $defaultShippingOption) test @else d-none @endif third_method">
        <div class="col-md-12">
            <div class="form-group"
                 style="overflow-x: scroll; background: white; padding: 10px">
                <h4 class="sub-title">Select your preferred date and time.</h4>
                <br>
                <table class="table table-bordered" id="timetable">
                    <thead>
                    <tr>
                        <?php for ($i = 0; $i < 7; $i++) { ?>
                        <th><?= $viewData['weekDays'][$i] ?> <?= $viewData['weekDates'][$i] ?></th>
                        <?php } ?>
                    </tr>
                    </thead>
                    <tbody>
                    <?php for ($i = 0; $i < $viewData['max_count']; $i++){ ?>
                    <tr>
                        <?php for($j = 0; $j < 7; $j++){ ?>
                        <?php if(isset($viewData['delivery_times'][$j][$i])) { ?>
                        <?php if($viewData['delivery_times'][$j][$i]->order_count < $viewData['delivery_times'][$j][$i]->delivery_per_slot){
                        ?>
                        <td style="cursor: pointer;"
                            data-deliverytime_id="<?= $viewData['delivery_times'][$j][$i]->id ?>"
                            data-delivery_date="<?= $viewData['delivery_dates'][$j] ?>"
                            onclick="selDeliveryTime(this);"><?= $viewData['delivery_times'][$j][$i]->time ?></td>
                        <?php } else { ?>
                        <td style="background: red; color: white">Full</td>
                        <?php } ?>
                        <?php } else { ?>
                        <td></td>
                        <?php } ?>
   ...
   <div class="position-relative">
       ...
       <input type="hidden" name="sub_contra_dis" value="{{ $dis }}">
...
{{--               @dd(setting('payment_cod_name'))--}}
           ...
           @endif
           @if(auth('customer')->user())
               @if(auth('customer')->user()->ranking == 4 || auth('customer')->user()->ranking == 3 || auth('customer')->user()->ranking == 2)
           @if (setting('payment_send_for_corporate_approval_status') == 1)
               <li class="list-group-item">
                   <input class="magic-radio js_payment_method" type="radio" name="payment_method" id="payment_sfcad"
                          @if (setting('default_payment_method') == \Botble\Payment\Enums\PaymentMethodEnum::SendForCoporateApproval) checked @endif
                          value="send_for_coporate_approval" data-bs-toggle="collapse" data-bs-target=".payment_sfcad_wrap" data-parent=".list_payment_method">
                   <label for="payment_sfcad" class="text-start">{{ setting('payment_send_for_corporate_approval_name', 'Send For Corporate Approval') }}</label>
                   <div class="payment_sfcad_wrap payment_collapse_wrap collapse @if (setting('default_payment_method') == \Botble\Payment\Enums\PaymentMethodEnum::SendForCoporateApproval) show @endif" style="padding: 15px 0;">
                       {!! clean(setting('payment_send_for_corporate_approval_description')) !!}
...
<script>
    function selDeliveryTime(cell) {
        var deliveryTimeId = 0;
        var deliveryDate = "";
        if ($(cell).css('background-color') == "rgb(0, 128, 0)") {
            $("#timetable td").css('background-color', "rgba(0, 0, 0, 0)");
            $("#timetable td").css('color', "#737373");
        } else {
            $("#timetable td").css('background-color', "rgba(0, 0, 0, 0)");
            cell.style.backgroundColor = "rgb(0, 128, 0)";
            deliveryTimeId = $(cell).attr('data-deliverytime_id');
            deliveryDate = $(cell).attr('data-delivery_date');
            $("#timetable td").css('color', "#737373");
            $(cell).css('color', "#fff");
        }
        $("#delivery_id").val(deliveryTimeId);
        $("#delivery_dt").val(deliveryDate);
    }
</script>
